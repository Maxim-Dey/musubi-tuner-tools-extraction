...
The user copies a separate example, replaces clearly marked server model paths and `TOK`, supplies their own captioned images, and starts training from the training TOML. The run prepares train and validation caches automatically without editing existing user configurations.
