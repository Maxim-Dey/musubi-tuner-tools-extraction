...
Deliver four physical example files in a separate portable experiment folder, plus exact cache, train, monitor, move, and resume instructions. Use the existing Qwen-Image original LoRA entrypoints and Stage 1–3 path, validation, and complete-state contracts. Verify the example through real parsers and readers and one focused controlled CPU integration. No production abstraction or training-math change is planned.
...
**Scale/Scope**: One Qwen-Image original LoRA example, three dataset roles, four physical files, four cache invocations, one training invocation and two resume examples; targeted CPU checks only.
...
| II. Task Fidelity | PASS: Preserve exact requested values and Stage 1–3 behavior. The only agreed value correction is save_precision=fp32 for exact single-file resume, explained in the example and guide. |
| III. Minimal, Compatible Changes | PASS: Reuse existing CLI readers, configuration paths, and test infrastructure. No new production layer, parser alias, or behavior outside Qwen opt-in. |
...
| V. Memory and Performance | PASS: One adapter per package; fixtures stay small. No H200 memory or speed claim. |
...
| Local Stage and Workflow | PASS: Small CPU fixture models are allowed for controlled tests; no full Qwen model weights/loading, GPU training, download, server access, or code implementation in this planning step. |
...
### Documentation and deliverables
    ...
    qwen_image_lora_val_example/
      ...
      cache/train/            # cache CLI fills these after data preparation
      cache/val_familiar/
...
1. The trainer selects root/train.toml with --config_file; each cache command either selects root/train.toml with --train_config or uses an absolute --experiment_dir. Each cache call selects one canonical dataset TOML and explicitly supplies --vae or --text_encoder. The training TOML does not import cache model paths.
2. The three model references in train.toml are clearly replaceable absolute server paths to original BF16 DiT, VAE and text encoder. The example is a template until user data, captions, model paths and caches exist.
...
| dit, vae, text_encoder | Trainer preflight and model readers; cache --vae/--text_encoder supplied separately | replaceable absolute server files; original BF16 DiT, not FP8 conversion |
...
1. Write the four example files with exact values from FR-002–FR-007 and create only the agreed directory structure. Keep real images, caption files, model paths, and caches user-supplied. Verify no role-bearing entry in train-dataset.toml and no random transform in val-dataset.toml.
...
- **Path and CLI gate**: The four cache invocations use real accepted flags; each val invocation processes both roles; root relocation and other-CWD invocation preserve effective paths and Stage 1 identity.
