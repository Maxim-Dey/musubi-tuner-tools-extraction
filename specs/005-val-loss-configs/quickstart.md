...
Expected: the trainer accepts `--config_file`; both cache commands accept `--train_config`, `--dataset_config`, `--experiment_dir`, and their respective explicit `--vae` or `--text_encoder`. Cache `--train_config` supplies the root anchor only; it does not import the model paths from the training TOML.

## 3. Operational command sequence after supplying real inputs

These commands are a documented later server procedure. They are not part of the local CPU check. Set `REPO`, `ROOT`, `VAE`, and `TEXT_ENCODER` to real absolute paths. The shell starts outside the repository and experiment to prove that invocation CWD is not the path anchor.

```bash
REPO=/workspace/musubi-tuner-tools-extraction
ROOT=/workspace/qwen_image_lora_val_example
VAE=/replace/with/server/qwen_image_vae.safetensors
TEXT_ENCODER=/replace/with/server/qwen_text_encoder.safetensors
...
The two validation cache invocations each process both role-labelled sets in `val-dataset.toml`. The example requires sources and caches to be ready before trainer validation preflight. The literal placeholder paths shown above must be replaced; these commands are not evidence of a run.
