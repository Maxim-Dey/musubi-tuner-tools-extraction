...
| --- | --- | --- |
| Deliver a separate `qwen_image_lora_val_example/` with four user-facing files and empty data/cache locations only. | The experiment files must not overwrite user inputs or imply fabricated images/caches are usable. Numerical training controls are editable by the user. | The removed `config_for_qwen_image_lora/` folder is intentionally absent. |
